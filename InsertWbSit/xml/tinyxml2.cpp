    ...
    if ( *(p+1) == '#' && *(p+2) ) {
        ...
        if ( *(p+2) == 'x' ) {
            ...
            while ( *q != 'x' ) {
                ...
                //TIXMLASSERT( ucs <= Usize_t_MAX - digitScaled );
                ucs += digitScaled;
...
// This is likely overengineered template art to have a check that unsigned size_t value incremented
// by one still fits into size_t. If size_t type is larger than unsigned size_t type
// (x86_64-w64-mingw32 target) then the check is redundant and gcc and clang emit
// -Wtype-limits warning. This piece makes the compiler select code with a check when a check
// is useful and code with no check when a check is redundant depending on how size_t and unsigned size_t
// types sizes relate to each other.
template
<bool = (sizeof(unsigned long) >= sizeof(long))>
...
XMLError XMLDocument::LoadFile( FILE* fp )
{
    ...
    if ( !longFitsIntoSizeTMinusOne<>::Fits( (unsigned long)filelength ) ) {
